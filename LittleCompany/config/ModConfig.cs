...
namespace LittleCompany.Config
{
    public sealed class ModConfig
    {
        ...
        public void Setup()
        {
            ...
            values.itemSizeChangeStep           = Plugin.BepInExConfig().Bind("Sizing", "ItemSizeChangeStep", 0.5f, new ConfigDescription("Defines how much an item shrinks/enlarges in one step.", new AcceptableValueRange<float>(SmallestSizeChange, 10f))).Value;
            values.enemySizeChangeStep          = Plugin.BepInExConfig().Bind("Sizing", "EnemySizeChangeStep", 0.5f, new ConfigDescription("Defines how much an enemy shrinks/enlarges in one step.", new AcceptableValueRange<float>(SmallestSizeChange, 10f))).Value;
